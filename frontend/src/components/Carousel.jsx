...
const LandingPageCarousel = () => {
  ...
  const slides = [placeholder, placeholder2, placeholder];
  ...
  return (
    <div id="carousel" className="relative w-full px-2">
      {/* Carousel Wrapper */}
      <div className="relative h-56 overflow-hidden rounded-2xl md:rounded-xl md:h-96">
      ...
      <div className="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3">
        {slides.map((_, index) => (
          <button
            key={index}
            type="button"
            className={`w-3 h-3 rounded-full ${currentSlide === index ? 'bg-gray-400' : 'bg-white'}`}
